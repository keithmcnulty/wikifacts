...
- `wiki_query()` sends SPARQL queries to the Wikibase retrieves results in a dataframe.
...
You can send SPARQL queries to Wikibase using `wiki_query()` and retrieve the results in a dataframe. If you have never queried Wikibase before, [here](https://query.wikidata.org/) is a good starting point to construct SPARQL queries and you can find lots of examples [here](https://www.wikidata.org/wiki/Wikidata:SPARQL_query_service/queries/examples).

In this example, a bar chart is created to show the top ten countries according to the number of cities with female mayors, according to data in Wikibase:
...
Or, a more darker topic, the top twenty countries by number of serial killers born there:
